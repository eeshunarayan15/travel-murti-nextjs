...
export default function NavbarClient({ packages }: { packages: any[] }) {
  const [subPackages, setSubPackages] = useState<Record<string, any[]>>({});
    ...
    async (packageId: string) => {
      ...
      try {
       const res = await fetch(
         `${process.env.NEXT_PUBLIC_API_URL || "https://travel-murti.onrender.com/api"}/subpackages/package/${packageId}`,
       );
  ...
  const openDesktopDropdown = (pkgId: string) => {
    clearTimeout(dropdownCloseTimer.current ?? undefined);
  ...
  const toggleMobileDropdown = (pkgId: string) => {
  ...
  const getDropdownButtonClass = (pkgId: string) => {
    const base =
